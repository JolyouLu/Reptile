                ...
                case '2':
                    ...
                    if (0 == $('#' + oSettings.layer_id).length) {
                        switch (oSettings.layer_append_type) {
                            case '1':
                                oSettings.oLayerElement = $('<div />').attr({
                                    'id': oSettings.layer_id,
                                    'class': oSettings.layer_class_name
                                }).appendTo(oSettings.oBindElement);
                                break;

                            case '2':
                                oSettings.oLayerElement = $('<div />').insertAttr({
                                    'id': oSettings.layer_id,
                                    'class': oSettings.layer_class_name
                                }).insertAfter(oSettings.oBindElement);
                                break;
                        }
                    } else {
                ...
                default:
                    if (0 == $('#' + oSettings.layer_id).length) {
                        oSettings.oLayerElement = $('<div />').attr({
                            ...
                            'display': 'none',
                            'position': 'absolute',
                            'z-index': oSettings.layer_z_index
                        }).appendTo('body');
                    } else {
                        oSettings.oLayerElement = $('#' + oSettings.layer_id);
                    }
                    ...
                    if (0 == $('#' + oSettings.layer_back_drop_id).length) {
                        oSettings.oBackDropElement = $('<div />').attr({
                            'id': oSettings.layer_back_drop_id,
                            'class': oSettings.layer_back_drop_class
                        }).css({
                            'z-index': oSettings.layer_back_drop_z_index,
                            'position': 'absolute',
