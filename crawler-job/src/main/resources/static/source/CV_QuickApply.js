...
function saveRsmKeyAndSend(domain_my) {
    ...
    if ($('#GrRsmSelectedKeyword').find('span').length > 0) {
        ...
        if (cansave) {
            resumekey = $.trim(resumekey);
            $.getJSON(domain_my + '/cv/in/Resume/GrassRootsResumeAction.php?type=2&jsoncallback=?', {
                r: Math.random,
                jobid: jobid,
                resumeid: $("#rsmid").val(),
                resumekey: resumekey
        ...
        $.getJSON(url, {type: "mobilePhone", value: phoneNum}, function (result) {
            if (result == 1) {
                ...
                document.getElementById("btn").onclick = function () {
                    sendVerifyCode(DOMAIN_MY, imgpath, language)
                }
                $('#Mobile').parent().removeClass('active');
...
function sendPhoneVerifyCode(DOMAIN_MY, imgpath, language) {
    ...
    //ajax 发送手机验证码短信
    $.getJSON(DOMAIN_MY + "/AjaxAction/mobile_code/send_mobile_code.php?rand=" + Math.random() + "&jsoncallback=?", {
        mobile: phoneNum,
        apptype: 7
    }, function (result) {
        // result = eval('(' + result + ')');
...
function bandGrassRootResumeInput() {
    ...
    {
        $('#YearOfBirthday_div').yearLayer({
            'id': 'YearOfBirthday',
            'div_id': 'YearOfBirthday_div',
            'code_id': 'YearOfBirthday',
            'layer_id': 'YearOfBirthday_layer',
            'data_max': -10,
            "after_close": function () {
            }
        });
        $('#MonthOfBirthday_div').monthLayer({
            'id': 'MonthOfBirthday',
            'div_id': 'MonthOfBirthday_div',
            'code_id': 'MonthOfBirthday',
            'layer_id': 'MonthOfBirthday_layer',
            "after_close": function () {
            }
        });
        $('#DayOfBirthday_div').dayLayer({
            'id': 'DayOfBirthday',
            'div_id': 'DayOfBirthday_div',
            'code_id': 'DayOfBirthday',
            'layer_id': 'DayOfBirthday_layer',
            "after_close": function () {
            }
        });
    }
    ...
    {
        $('#MPNation_div').mobileNationLayer({
            'id': 'MPNationText',
            'div_id': 'MPNation_div',
            'code_id': 'MPNation',
            'layer_id': 'MPNation_layer',
            "after_close": function () {
            }
        });
    }
    $('#WorkYear_div').workyearLayer({
        'id': 'WorkYearText',
        'div_id': 'WorkYear_div',
        'code_id': 'WorkYear',
        'layer_id': 'WorkYear_layer',
        "after_close": function () {
        }
    });
    //居住地
    $('#grRsmArea_btn').areaLayer({
        'id': 'grRsmAreaText',
        "code_id": "grRsmAreaVal",
        'div_id': 'grRsmArea_div',
        'data_district_show': true,
        'layer_index': '2000',
        'back_drop_index': '1999',
        "after_close": function () {
        }
    });
    $('#grRsmArea_div').areaSearch({
        'id': 'grRsmAreaText',
        "code_id": "grRsmAreaVal",
        'layer_id': 'grRsmArea_layer',
        'div_id': 'grRsmArea_div',
        'data_show_type': 7,
        "after_close": function () {
        }
    ...
    //目标职位选择
    $('#grRsmFunc_btn').funtypeLayer({
        "id": "grRsmFuncText",
        "code_id": "grRsmFuncVal",
        'div_id': 'grRsmFunc_div',
        'layer_index': '2000',
        'back_drop_index': '1999',
        "after_close": function () {
        }
    });
    $('#grRsmFunc_div').funtypeSearch({
        "id": "grRsmFuncText",
        "code_id": "grRsmFuncVal",
        'div_id': 'grRsmFunc_div',
        "layer_id": "grRsmFunc_layer",
        "after_close": function () {
            if ($('#funtype_err').is(':visible')) {
                chkFuntype();
            }
        }
    });
